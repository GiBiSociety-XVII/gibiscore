...
function Stat({value, label}: {value: string | number; label: string}) {
    return (
        <div className="border-2 border-foreground rounded-xl p-3 flex flex-col gap-1 bg-background">
            <span className="font-mono text-[28px] font-bold leading-none tabular-nums">{value}</span>
...
export function PlayerSpotlight({player}: {player: PlayerSpotlightData}) {
    ...
    return (
        <Card press className="p-6 flex flex-col gap-4">
            <h2 className="text-[22px] font-extrabold tracking-tight">{t('title')}</h2>
            <div className="flex items-center gap-4">
                <span className="flex w-16 h-16 rounded-xl border-[2.5px] border-foreground bg-muted overflow-hidden shrink-0">
                    {player.imageUrl && (
                        <Image src={player.imageUrl} alt={player.name} width={64} height={64} className="object-cover" />
                    )}
                </span>
                <div className="flex flex-col gap-1 min-w-0">
                    <span className="text-lg font-extrabold truncate">{player.name}</span>
