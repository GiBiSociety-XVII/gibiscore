...
function Rating({value, accent = 7}: {value: number | null; accent?: number}) {
    ...
    return (
        <span className={cn("inline-block font-mono text-xs font-extrabold tabular-nums px-1.5 py-0.5 rounded-md border-2 border-foreground", value >= accent ? "bg-accent" : "bg-card")}>
            {value.toFixed(1)}
        </span>
    );
}
                        ...
                        {lines.map((l) => {
                            ...
                            return (
                                <tr key={l.player.id} className={cn("border-t-2 border-muted font-semibold", (l.minutes ?? 0) === 0 && "text-muted-foreground")}>
                                    ...
                                    <td className="px-1.5 py-2 text-right font-mono tabular-nums">{l.minutes ?? '–'}</td>
                                    <td className="px-1.5 py-2 text-right"><Rating value={l.rating} /></td>
                                    <td className="px-1.5 py-2 text-right"><Rating value={l.fantasy} accent={7.5} /></td>
                                    <td className="px-1.5 py-2 text-right font-mono tabular-nums">{l.goals || ''}</td>
                                    <td className="px-1.5 py-2 text-right font-mono tabular-nums">{l.assists || ''}</td>
                                    <td className="px-1.5 py-2 text-right font-mono tabular-nums hidden md:table-cell">{l.shots ?? ''}{l.shotsOnTarget !== null && l.shots ? ` (${l.shotsOnTarget})` : ''}</td>
                                    <td className="px-1.5 py-2 text-right font-mono tabular-nums hidden md:table-cell">{l.keyPasses ?? ''}</td>
