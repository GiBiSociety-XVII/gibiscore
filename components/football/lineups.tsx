...
function PlayerLine({p}: {p: LineupPlayer}) {
    ...
    return (
        <li className="flex items-center gap-2 py-1.5 border-t-2 border-muted first:border-t-0">
            ...
            {p.rating !== null && (
                <span className={cn("ml-auto font-mono text-xs font-extrabold tabular-nums px-1.5 py-0.5 rounded-md border-2 border-foreground", p.rating >= 7 ? "bg-accent" : "bg-card")}>
                    {p.rating.toFixed(1)}
                </span>
            )}
        </li>
    ...
    return (
        <div className="flex flex-col gap-3 min-w-0">
            <div className="flex items-center gap-2">
                <TeamCrest team={lineup.team} size={28} />
