...
export function MatchList({fixtures, title, highlightTeamId, showDate = false, showCompetition = false}: {fixtures: FixtureSummary[]; title?: string; highlightTeamId?: number; showDate?: boolean; showCompetition?: boolean}) {
    ...
    // A column for the odds when some match of the list has them: the rows of the list align on it.
    const oddsColumn = fixtures.some((f) => f.state === 'scheduled' && !!f.odds);
    return (
        <section className="bb-surface overflow-hidden">
            ...
            {fixtures.length === 0 ? (
                ...
                <div className="flex flex-col">
                    {fixtures.map((f) => <MatchRow key={f.id} fixture={f} highlightTeamId={highlightTeamId} showDate={showDate} showCompetition={showCompetition} oddsColumn={oddsColumn} />)}
                </div>
            )}
        </section>
    );
