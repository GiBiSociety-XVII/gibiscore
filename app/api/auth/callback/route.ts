...
export async function GET(request: NextRequest) {
    ...
    // `next` carries the language of the page that sent the mail; without it, the site's own.
    const safeNext = safeNextPath(request.nextUrl.searchParams.get('next') ?? undefined, routing.defaultLocale);
    if (code) {
