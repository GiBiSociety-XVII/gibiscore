...
export async function POST(request: NextRequest) {
    let body: {endpoint?: unknown; keys?: {p256dh?: unknown; auth?: unknown}; userAgent?: unknown; locale?: unknown};
    try {
    ...
    // The language of the page that asked: the notifications of this browser are written in it.
    ...
    const {error} = await db.from('push_subscriptions').insert({user_id: who.user.id, endpoint, p256dh, auth, locale, user_agent: typeof body.userAgent === 'string' ? body.userAgent.slice(0, 300) : null});
