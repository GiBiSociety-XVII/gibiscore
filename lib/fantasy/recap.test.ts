...
import {bestHindsight, defenceModifierOf, playLineup, roundPoints, surprises, votoOf, type RoundResults, type RoundStat} from './recap';
...
const official = (stats: Record<number, RoundStat>): RoundResults => ({round: 'Regular Season - 3', official: true, stats});

describe('votoOf', () => {
    it('takes the official vote when the round is in, the rating on the vote scale otherwise', () => {
        expect(votoOf(stat({voto: 6.5, rating: 7.4}), 'C', true, DEFAULT_CALIBRATION)).toBe(6.5);
        expect(votoOf(stat({voto: null, rating: 7.4}), 'C', true, DEFAULT_CALIBRATION)).toBeNull();
        const estimated = votoOf(stat({rating: 7.4}), 'C', false, DEFAULT_CALIBRATION)!;
    ...
    it('gives no estimated vote to who barely played or was not rated', () => {
        expect(votoOf(stat({rating: 7, minutes: 4}), 'A', false, DEFAULT_CALIBRATION)).toBeNull();
        expect(votoOf(stat({rating: null}), 'A', false, DEFAULT_CALIBRATION)).toBeNull();
        expect(votoOf(undefined, 'A', false, DEFAULT_CALIBRATION)).toBeNull();
    });
});

describe('roundPoints', () => {
    it('pays the events under the rules; keepers their conceded, saves and clean sheet', () => {
        expect(roundPoints(stat({voto: 6.5, goals: 2, assists: 1, yellow: 1}), 'A', CLASSIC_RULES, true, DEFAULT_CALIBRATION)).toBe(13);
        expect(roundPoints(stat({voto: 6, conceded: 2}), 'P', CLASSIC_RULES, true, DEFAULT_CALIBRATION)).toBe(4);
        expect(roundPoints(stat({voto: 6.5, conceded: 0, penaltiesSaved: 1}), 'P', CLASSIC_RULES, true, DEFAULT_CALIBRATION)).toBe(10.5);
        // Outfield players never pay the goals conceded.
        expect(roundPoints(stat({voto: 6, conceded: 3, ownGoals: 1}), 'D', CLASSIC_RULES, true, DEFAULT_CALIBRATION)).toBe(4);
        expect(roundPoints(stat({voto: null}), 'D', CLASSIC_RULES, true, DEFAULT_CALIBRATION)).toBeNull();
    });
});
