...
describe('detectChanges', () => {
    ...
    it('reports the kick-off once the match leaves the schedule', () => {
        ...
        expect(out[0].title).toBe("Calcio d'inizio: Juventus – Inter");
    });
    it('reports half-time and full-time with the score', () => {
        expect(detectChanges({state: 'live', homeScore: 1, awayScore: 0}, facts({state: 'half_time', minute: 45}))[0]).toMatchObject({kind: 'half_time', title: 'Fine primo tempo: Juventus 1-0 Inter'});
        expect(detectChanges({state: 'live', homeScore: 2, awayScore: 1}, facts({state: 'finished', minute: 90, homeScore: 2, awayScore: 1}))[0]).toMatchObject({kind: 'full_time', key: 'full_time', title: 'Finale: Juventus 2-1 Inter'});
    ...
    it('names the scorer and the way of the goal', () => {
        ...
        expect(detectChanges(prev, facts({events: [goal()]}))[0]).toMatchObject({kind: 'goal', key: 'goal:496:28+0:Vlahović', title: '⚽ Gol Juventus! Juventus 1-0 Inter', body: "28' Vlahović · Serie A"});
        expect(detectChanges(prev, facts({events: [goal({kind: 'penalty', teamProviderId: 505, player: 'Çalhanoğlu'})]}))[0].title).toBe('⚽ Gol Inter su rigore! Juventus 1-0 Inter');
        expect(detectChanges(prev, facts({events: [goal({kind: 'own_goal'})]}))[0].title).toContain('(autogol)');
    });
    it('keeps a fresh goal in stoppage time and drops a stale one', () => {
        ...
        expect(out[0].body.startsWith("45+3'")).toBe(true);
    });
    it('reports a red card', () => {
        ...
        expect(out[0]).toMatchObject({kind: 'red_card', key: 'red:496:68:Bremer', title: '🟥 Espulso Bremer (Juventus)'});
    });
